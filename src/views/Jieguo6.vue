          ...
          <div>
            ...
            <div class="integral-box" v-if="!scaleBo" >
                <div class="img-box"   ref="canvasImg":style="{'display': imgLoadBo?'block':'none'}">
                  <p class="name"><span class="sp1">{{name}}</span> <span class="sp2">的TA是</span></p> 
...
export default class Jieguo2 extends Mixins(MixinUtil) {
  ...
  get url() {
    return `//yxxl.net.cn/banner/10004/${+this.integral}.jpg`;
  }
  ...
  data(){
    ...
    let himgsrc="//yxxl.net.cn/banner/10004/"+num+".png"
    console.log(himgsrc)
  ...
  created() {
    ...
    if(this.sex == 1){
             this.numnv=Math.floor((Math.random()*8+9))
             this.integral=this.numnv
             console.log(this.integral)
    }else if(this.sex == 2){
             this.numnan=Math.floor((Math.random()*8+1))
             this.integral=this.numnan
             console.log(this.integral)
